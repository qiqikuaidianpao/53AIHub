...
/**
 * RAG Statistics Formatting Hook
 */
export function useRagStats() {
  const formatRagStats = useCallback((ragStats: any, processRecords: any[] = []) => {
    const knowledgeSearch = parseJson((processRecords.find((record: any) => record.step_code === 'knowledge_search' && record.status === 'completed') || { data: '{"sources":[]}' }).data, { sources: [] })
    ...
    const filesSearch = chunks
      .filter((item: any) => ['web_search', 'web_page', 'knowledge', 'knowledge_search', 'summary', 'knowledge_map', 'graph_result'].includes(item.chunk_type))
      .map((chunk: any) => {
    ...
    const documentQuotations = document_quotations.map((chunk_id: any) =>
      filesSearch.find((item: any) => item.chunk_id === String(chunk_id))
    ).filter(Boolean)
    ...
    return ragStats
      ? {
          ...ragStats,
          chunks: filesSearch,
          library_search: libraryIds.map((id: any) => filesSearch.find((chunk: any) => chunk.library_id === id)).filter(Boolean),
          files_search: fileIds.map((id: any) => filesSearch.find((chunk: any) => chunk.file_id === id)).filter(Boolean),
